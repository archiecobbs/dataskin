...
public class BasicDisplayHooks implements DisplayHooks {
    ...
    public BasicDisplayHooks(OperationCache operationCache, Function<? super Runnable, ? extends Future<?>> executor) {
    ...
    public <C, R> void displayActionResult(UI ui, Action<C, R> action, C config, R result) {
        VaadinUtil.assertCurrentSession();
        final String message = String.format("%s successful", action.getLabel());
        if (result == null)
    ...
    public <C, R> void displayOperationError(UI ui, Operation<C, R> operation, C config, Throwable error) {
        VaadinUtil.assertCurrentSession();
        final String message = String.format("%s failed", operation.getLabel());
...
// FieldBuilderContext
    ...
    public static class FieldBuilderContext extends FieldBuilderContextImpl {
        ...
        public OperationCache getOperationCache() {
            return this.displayHooks.operationCache;
        }
    }
}
